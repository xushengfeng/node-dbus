...
export class dbusServer {
    ...
    private handlers: Map<string, Map<string, Map<string, ServerMethodHandler>>> = new Map();
    ...
    addObject(path: string, iface: string, methods: Record<string, ServerMethodHandler>): void {
    ...
    getHandler(path: string, iface: string, method: string): ServerMethodHandler | undefined {
    ...
    async emitSignal(path: string, iface: string, name: string, signature: string = "", args: unknown[] = []): Promise<void> {
    ...
    private async handleMessage(msg: dbusMessage): Promise<void> {
        ...
        try {
            ...
            if (result !== undefined) {
                if (result && typeof result === "object" && "signature" in result && "values" in result && Array.isArray(result.values)) {
